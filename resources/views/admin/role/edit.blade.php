    ...
    <div id="mytask-layout" class="theme-indigo">
        ...
        <div class="main px-lg-4 px-md-4 mt-4">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form id="advanced-form" action="{{ route('admin.role.update',['id' => $role->role_id]) }}" method="POST" data-parsley-validate novalidate>
                        @csrf
                        <div class="row g-3 align-items-center">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <br />
                                    <label class="form-label"><strong>Role Detail</strong></label>
                                    <input type="text" id="role_detail" name="role_detail" 
                                    value="{{ old('role_detail', $role->role_detail) }}"
                                    class="form-control">
                                    <label class="form-label mt-2"><strong>Status</strong></label>
                                    <br />
                                        <td>
                                            <input type="radio" name="status" value="1" {{ old('status', $role->status) == 1 ? 'checked' : '' }}> Active
                                            <input type="radio" name="status" value="2" {{ old('status', $role->status) == 2 ? 'checked' : '' }}> Block
                                        </td>
                                    <br />
                                    <label class="form-label mt-2"><strong>View rights</strong></label>
                                    <br />
                                        <td>
                                            <input type="radio" name="view_right" value="1" {{ old('view_right', $role->view_right) == 1 ? 'checked' : '' }}> Admin view
                                            <input type="radio" name="view_right" value="2" {{ old('view_right', $role->view_right) == 2 ? 'checked' : '' }}> All view
                                            <input type="radio" name="view_right" value="3" {{ old('view_right', $role->view_right) == 3 ? 'checked' : '' }}> Employee view
                                        </td>
                                    <br />
                                </div>
                        </div>
